...
public class ReservationProjectDaoSqls {
	...
	public static final String DETAIL_FILEINFO = "SELECT prod.description, prod.content, fi.save_file_name FROM product prod JOIN product_image pi ON (prod.id = pi.product_id) JOIN file_info fi ON (pi.file_id = fi.id) WHERE (pi.type = 'ma' OR pi.type = 'et') AND prod.id = :productId";
