...
public class ReservationProjectDaoSqls {
	...
	public static final String RESERVATION_INFO = "SELECT ri.product_id, ri.display_info_id, prod.description, di.opening_hours, di.place_name, di.tel, di.homepage, ri.reservation_email, ri.cancel_flag FROM product prod JOIN reservation_info ri ON (ri.product_id = prod.id) JOIN display_info di ON (di.product_id = ri.display_info_id) WHERE ri.reservation_email = :email";
