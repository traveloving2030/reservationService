...
public class ReservationProjectDaoSqls {
	...
	public static final String RESERVATION_INFO = "SELECT ri.id, ri.product_id, ri.display_info_id, prod.description, di.opening_hours, di.place_name, di.tel, di.homepage, ri.reservation_email, ri.reservation_date, ri.cancel_flag FROM product prod JOIN reservation_info ri ON (ri.product_id = prod.id) JOIN display_info di ON (di.product_id = ri.display_info_id) WHERE ri.reservation_email = :email";
	public static final String RSERVATION_GROUPBY_PRICE_TYPE = "SELECT prod.id, SUM(rip.count), fare.price_type_name, fare.price, fare.discount_rate, ri.reservation_email FROM product prod JOIN reservation_info ri ON (prod.id = ri.product_id) JOIN product_price fare ON (prod.id = fare.product_id) JOIN reservation_info_price rip ON (rip.reservation_info_id = ri.id AND rip.product_price_id = fare.id) WHERE ri.reservation_email = :email  GROUP BY price_type_name";
